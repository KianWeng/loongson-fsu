...
unsigned short findDevice(unsigned short device,
				 unsigned short vendor, int num);

unsigned short findClass(unsigned long classid,  int num);
		...
		switch (reg_al) {
			case 0x01:	// installation check
				if (PCI_IsInitialized()) {
					...
					reg_edx=0x20494350;
					reg_edi=0;//PCI_GetPModeInterface();
					CALLBACK_SCF(false);
				} else {
					CALLBACK_SCF(true);
				}
			...
			case 0x02: {	// find device
			    ...
			    found = findDevice(reg_cx,reg_dx,reg_si);
			...
			case 0x03: {	// find device by class code
				Bitu found;
				found = findClass(reg_ecx, reg_si);
				if (found!=0xffff) {
					reg_ah=0x00;
					reg_bx=found;
					CALLBACK_SCF(false);
