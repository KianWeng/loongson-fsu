...
// #define MARVELL_PHY_ID_88E6071		0x00000710
#define MARVELL_PHY_ID_88E6071		0x001CC915
...
{
#if 0
	int err, value, index = 0;
	unsigned long dat;
	void *addr = (void *)0xBFD00424; 

	// for(; index < 5; ++ index) {	
		// err = m88E6071_smi_chip_write(phydev, smi_device_addr[SEL_CHI], 0, 0x0000);
		
		// open isolate, power down, open auto-anen
		value = m88E6071_smi_chip_read(phydev, smi_device_addr[SEL_CHI], 0);
		value = value | 0x0400 | 0x0800 | 0x1000;
		printk(KERN_INFO "claude test config power: 0x%x\n", value);
		phydev->autoneg = AUTONEG_ENABLE;
		err = m88E6071_smi_chip_write(phydev, smi_device_addr[SEL_CHI], 0, value);
		if (err < 0)
		{
			return err;
		}		
	
		// err = m88E6071_smi_chip_write(phydev, smi_device_addr[SEL_CHI], 20, 0x0001);
		// gmac0 link to 100M phy
		dat = __raw_readl(addr);
		dat |= 0x5;
		writel(dat, addr);
		...
		value &= ~0x0800;
		err = m88E6071_smi_chip_write(phydev, smi_device_addr[SEL_CHI], 0, value);	
		// err = m88E6071_smi_chip_write(phydev, smi_device_addr[SEL_CHI], 0, 0x1000 | 0x0200);	
		printk(KERN_INFO "claude test config init ok: 0x%x\n", value);
	//}
	/*
	int i = 0;
	int val = 0;
	for(; i < 5; ++i) {
		val = m88E6071_smi_chip_read(phydev, smi_device_addr[i], 3);
		printk(KERN_INFO "claude test device 0x%8x port status 0x%8x\n", smi_port_device_addr[i], val);
	}
	*/
	return err;
#else
	writel(readl((void *)0xbfd00424)|5,  (void *)0xbfd00424);
...
#if 0
	int err = 0;
	err = m88E6071_smi_chip_read(phydev, smi_device_addr[SEL_CHI], 4);
	err = m88E6071_smi_chip_write(phydev, smi_device_addr[SEL_CHI], 4, err | 0x0100);

	if(err > 0) {
		// restart aneg
		err = m88E6071_smi_chip_read(phydev, smi_device_addr[SEL_CHI], 0);
		err = err | 0x1000 | 0x0200;
		err &= ~0x0400;
		err = m88E6071_smi_chip_write(phydev, smi_device_addr[SEL_CHI], 0, err);
	}
	return err;
#else
...
static int m88E6071_read_status(struct phy_device *phydev)
{
	...
	val = m88E6071_smi_chip_read(phydev, smi_device_addr[SEL_CHI], 1);
	
	if(val & 0x4000) { 			/* FDF 100M */
		speed = SPEED_100;
		duplex = DUPLEX_FULL;	
	} else if (val & 0x2000){ 		/* HDF 100M */
		speed = SPEED_100;
		duplex = DUPLEX_HALF;
	} else if (val & 0x1000) { 		/* FDF 10M */
		speed = SPEED_10;
		duplex = DUPLEX_FULL;
	} else {					/* HDF 10M */
		speed = SPEED_10;
...
//	phydev->link = 1;	
	...
	//for(; index < 5; ++index) {
		val = m88E6071_smi_chip_read(phydev, smi_device_addr[SEL_CHI], 1);
		if(val != oldval)
			printk(KERN_INFO "claude test %d status 0x%8x\n", index, val);		
		oldval = val;
	//}
...
{
	int err = m88E6071_smi_chip_read(phydev, smi_device_addr[SEL_CHI], 18);
	return (err < 0) ? err : 0;
}
...
{
	return phy_drivers_register(marvell_drivers, ARRAY_SIZE(marvell_drivers));
}

static void __exit marvell_exit(void)
{
	return phy_drivers_unregister(marvell_drivers, ARRAY_SIZE(marvell_drivers));
}
